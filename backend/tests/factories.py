...
class CourseFactory(BaseFactory):
    ...
    grade_level = factory.fuzzy.FuzzyChoice([
        "小学", "初中", "高中", "大学"
    ])
...
class ProjectFactory(BaseFactory):
    ...
    resources = factory.LazyFunction(
        lambda: [fake.url() for _ in range(2, 5)]
    )
...
class PerformanceMetricFactory(BaseFactory):
    ...
    metric_name = factory.fuzzy.FuzzyChoice([
        "api_response_time", "database_query_time", "ai_processing_time",
        "websocket_latency", "memory_usage", "cpu_usage"
    ])
...
class DataGenerator:
    ...
    @staticmethod
    def create_courses_with_projects(course_count: int = 5, projects_per_course: int = 3):
        """创建课程和相关项目"""
        courses = []
