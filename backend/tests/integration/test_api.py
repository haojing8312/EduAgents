...
from tests.factories import CourseFactory, ProjectFactory, UserFactory
...
class TestCourseAPI:
    ...
    async def test_create_course(self, client: AsyncClient, authenticated_user):
        ...
        response = await client.post(
            "/api/v1/courses/", json=course_data, headers=headers
        )
...
class TestAgentAPI:
    ...
    async def test_agent_consultation(self, client: AsyncClient, mock_ai_service):
        ...
        consultation_data = {
            ...
            "context": {"grade_level": "高中", "duration": "12周", "class_size": 30},
        }
    ...
    async def test_multi_agent_collaboration(
        self, client: AsyncClient, mock_ai_service
    ):
        """测试多智能体协作"""
        collaboration_data = {
        ...
        response = await client.post(
            "/api/v1/agents/collaborate", json=collaboration_data
        )
...
class TestWebSocketAPI:
    ...
    async def test_websocket_connection(self, client: AsyncClient):
        ...
        async with client.websocket_connect("/ws/agents") as websocket:
            ...
            test_message = {
                "type": "agent_query",
                "data": {"agent": "education_director", "query": "测试查询"},
            }
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_websocket_agent_collaboration(
        self, client: AsyncClient, mock_ai_service
    ):
        """测试WebSocket智能体协作"""
        async with client.websocket_connect("/ws/collaboration") as websocket:
            ...
            assert all(
                msg["data"]["agent"] in ["education_director", "learning_designer"]
