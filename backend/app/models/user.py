...
class User(BaseModel):
    ...
    # 角色和权限
    role = Column(
        Enum(UserRole), default=UserRole.STUDENT, nullable=False, comment="用户角色"
    )
    ...
    organization = Column(String(200), nullable=True, comment="所属机构")
    ...
    def can_access_agent(self, agent_name: str) -> bool:
        ...
        role_permissions = {
            ...
            UserRole.STUDENT: ["learning_experience_designer", "creative_technologist"],
            UserRole.RESEARCHER: [
                "education_director",
                "pbl_curriculum_designer",
                "learning_experience_designer",
            ],
            UserRole.GUEST: ["learning_experience_designer"],
        }
...
class UserSession(BaseModel):
    ...
    session_token = Column(
        String(255), unique=True, nullable=False, index=True, comment="会话令牌"
    )
...
class UserPreference(BaseModel):
    ...
    value = Column(JSONB, nullable=True, comment="设置值")

    is_default = Column(Boolean, default=False, nullable=False, comment="是否为默认值")
