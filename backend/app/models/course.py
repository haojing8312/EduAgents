...
from sqlalchemy import Column, String, Text, Integer, Boolean, JSON, ForeignKey, Float, Table
...
course_collaborators = Table(
    ...
    Column('role', String(50), default='collaborator'),  # owner, editor, viewer, collaborator
...
class Course(BaseModel):
    ...
    # 课程属性
    status = Column(String(20), default=CourseStatus.DRAFT, nullable=False, comment="课程状态")
    difficulty_level = Column(String(20), default=DifficultyLevel.INTERMEDIATE, comment="难度等级")
    ...
    parent_course_id = Column(UUID(as_uuid=True), ForeignKey('courses.id'), nullable=True, comment="父课程ID")
    ...
    collaborations = relationship("User", secondary=course_collaborators, back_populates="collaborated_courses")
    ...
    # 子课程关系
    parent_course = relationship("Course", remote_side="Course.id", backref="child_courses")
...
class Lesson(BaseModel):
    ...
    course_id = Column(UUID(as_uuid=True), ForeignKey('courses.id'), nullable=False, comment="所属课程")
...
class Assessment(BaseModel):
    ...
    course_id = Column(UUID(as_uuid=True), ForeignKey('courses.id'), nullable=False, comment="所属课程")
    lesson_id = Column(UUID(as_uuid=True), ForeignKey('lessons.id'), nullable=True, comment="所属单元")
...
class CourseExport(BaseModel):
    ...
    course_id = Column(UUID(as_uuid=True), ForeignKey('courses.id'), nullable=False, comment="课程ID")
...
class CourseReview(BaseModel):
    ...
    course_id = Column(UUID(as_uuid=True), ForeignKey('courses.id'), nullable=False, comment="课程ID")
    reviewer_id = Column(UUID(as_uuid=True), ForeignKey('users.id'), nullable=False, comment="评价者ID")
