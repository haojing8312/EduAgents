...
from ...services.quality_checker import quality_checker, QualityLevel, CheckSeverity
            ...
            if course:
                if request.async_check:
                    ...
                    background_tasks.add_task(
                        _process_quality_check_async,
                        course_id,
                        course
                    )
                    results.append(QualityReportResponse(
                        course_id=course_id,
                        status="pending",
                        message="质量检查任务已创建"
                    ))
...
async def get_quality_statistics(
    ...
    # 计算统计信息
    scores = [course.quality_score for course in courses if course.quality_score is not None]
...
async def auto_improve_course(
    ...
    try:
        ...
        return {
            ...
            "new_score": course.quality_score if improvements_made else quality_report.overall_score,
            "message": f"成功应用{len(improvements_made)}项改进" if improvements_made else "未发现可自动改进的问题"
        }
...
def _apply_auto_improvement(course: Course, issue) -> Optional[Dict[str, Any]]:
    ...
    if "缺少学习目标" in issue.title and not course.learning_objectives:
        ...
        # 自动生成基础驱动性问题
        course.driving_question = f"如何运用{course.subject}知识解决我们身边的实际问题？"
        return {
            "type": "驱动性问题补充",
