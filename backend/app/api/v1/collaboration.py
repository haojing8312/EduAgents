...
async def get_course_collaborators(
    ...
    try:
        collaborators = await collaboration_service.get_course_collaborators(db, course_id)
...
async def check_user_permissions(
    ...
    try:
        ...
        for permission in Permission:
            permissions[permission.value] = await collaboration_service.check_permission(
...
async def get_my_collaborations(
    ...
    try:
        ...
        from ...models.course import course_collaborators, Course
        ...
        query = select(
            ...
            course_collaborators.join(Course, course_collaborators.c.course_id == Course.id)
        ).where(
...
async def get_collaboration_statistics(
    ...
    try:
        ...
        result = await db.execute(
            select(
                func.count(course_collaborators.c.course_id).label('total_collaborations'),
                course_collaborators.c.role
            ).where(
                course_collaborators.c.user_id == current_user.id
        ...
        recent_activity = await db.execute(
            select(func.count(Course.id)).select_from(
                course_collaborators.join(Course, course_collaborators.c.course_id == Course.id)
            ).where(
                course_collaborators.c.user_id == current_user.id,
        ...
        return {
            ...
            "most_active_role": max(role_stats.items(), key=lambda x: x[1])[0] if role_stats else None
        }
