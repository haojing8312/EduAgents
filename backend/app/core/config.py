...
class Settings(BaseSettings):
    ...
    def REDIS_SESSION_URL(self) -> str:
        """Redis会话连接URL"""
        auth = f":{self.REDIS_PASSWORD}@" if self.REDIS_PASSWORD else ""
        return f"redis://{auth}{self.REDIS_HOST}:{self.REDIS_PORT}/{self.REDIS_SESSION_DB}"
    
    # ChromaDB配置
    CHROMA_HOST: str = Field(default="localhost", env="CHROMA_HOST")
    CHROMA_PORT: int = Field(default=8000, env="CHROMA_PORT")
    CHROMA_COLLECTION_NAME: str = Field(default="pbl_knowledge", env="CHROMA_COLLECTION_NAME")
    ...
    # 大语言模型配置
    LLM_PROVIDER: str = Field(default="openai", env="LLM_PROVIDER")  # openai, anthropic, azure
