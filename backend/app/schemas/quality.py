...
from ..services.quality_checker import QualityLevel, CheckSeverity
...
class QualityReportResponse(BaseModel):
    ...
    category_scores: Dict[str, float] = Field(default_factory=dict, description="各类别得分")
...
class QualityComparisonRequest(BaseModel):
    """质量对比请求"""
    course_ids: List[UUID] = Field(..., min_items=2, max_items=5, description="要对比的课程ID")
...
class QualityCheckSchedule(BaseModel):
    """质量检查计划"""
    course_id: UUID = Field(..., description="课程ID")
    schedule_type: str = Field(..., description="计划类型")  # weekly, monthly, on_update
