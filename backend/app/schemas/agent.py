...
class AgentRequest(BaseModel):
    ...
    temperature: Optional[float] = Field(
        default=0.7, ge=0.0, le=2.0, description="回答随机性"
    )
    max_tokens: Optional[int] = Field(
        default=4000, ge=100, le=8000, description="最大令牌数"
    )
...
class AgentStatus(BaseModel):
    ...
    agent_name: str = Field(..., description="智能体名称")
    status: Literal["active", "busy", "maintenance", "offline"] = Field(
        description="状态"
    )
...
class AgentCollaborationRequest(BaseModel):
    ...
    collaboration_mode: CollaborationMode = Field(
        default=CollaborationMode.SEQUENTIAL, description="协作模式"
    )
    required_expertise: Optional[List[str]] = Field(
        default=None, description="所需专业技能"
    )
    ...
    timeout_minutes: int = Field(
        default=10, ge=1, le=60, description="超时时间（分钟）"
    )
...
class CollaborationResult(BaseModel):
    ...
    quality_score: Optional[float] = Field(
        default=None, ge=0, le=10, description="质量评分"
    )
...
class ConversationMessage(BaseModel):
    ...
    agent_name: Optional[str] = Field(
        default=None, description="智能体名称（如果是智能体消息）"
    )
...
class AgentMetrics(BaseModel):
    ...
    most_common_tasks: List[Dict[str, Union[str, int]]] = Field(
        ..., description="最常见任务"
    )
...
class AgentFeedback(BaseModel):
    ...
    feedback: Optional[str] = Field(
        default=None, max_length=1000, description="反馈内容"
    )
...
class WebSocketMessage(BaseModel):
    ...
    type: Literal["message", "status", "error", "ping", "pong"] = Field(
        ..., description="消息类型"
    )
    agent_name: Optional[str] = Field(default=None, description="智能体名称")
...
class BatchRequest(BaseModel):
    ...
    requests: List[AgentRequest] = Field(
        ..., min_items=1, max_items=100, description="请求列表"
    )
