...
from .state import AgentState, AgentMessage, MessageType, AgentRole, WorkflowPhase
...
class PBLOrchestrator:
    ...
    async def _theoretical_foundation_phase(self, state: AgentState) -> AgentState:
        ...
        if self.enable_streaming:
            ...
            async for res in result:
                state.theoretical_framework = res.get("content", {}).get("framework", {})
    ...
    async def _material_production_phase(self, state: AgentState) -> AgentState:
        ...
        material_types = [
            {"type": "create_worksheets", "specifications": {"modules": state.content_modules}},
    ...
    async def _broadcast_update(self, state: AgentState, update: Dict[str, Any]) -> None:
    ...
    async def design_course(
        ...
        """
        Main entry point for course design
        ...
        # Apply configuration
        if config:
            initial_state.streaming_enabled = config.get("streaming", True)
            self.max_iterations = config.get("max_iterations", 3)
        
        # Run the workflow
        if initial_state.streaming_enabled:
            # Stream results
    ...
    def _calculate_progress(self, state: AgentState) -> float:
        ...
        completed_weight = sum(
            phase_weights[phase]
            for phase in state.workflow_history
        )
