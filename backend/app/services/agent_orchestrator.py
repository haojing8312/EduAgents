...
from app.utils.cache import cache_result, CacheManager
from app.models.agent import ConversationSession, AgentInteraction
...
class AgentOrchestrator:
    ...
    def __init__(self):
        ...
        self.agent_capabilities = {
            ...
            "pbl_curriculum_designer": {
                ...
                "task_types": ["course_design", "project_planning", "assessment_design"],
                "priority": 2,
                "model": "opus"
            },
    ...
    def _build_collaboration_graph(self) -> StateGraph:
        ...
        graph.add_node("learning_experience_designer", self._execute_experience_designer)
    ...
    async def select_agents_for_task(
        ...
        if required_expertise:
            for agent_name, capabilities in self.agent_capabilities.items():
                if any(skill in capabilities["expertise"] for skill in required_expertise):
        ...
        selected_agents.sort(
            key=lambda x: self.agent_capabilities[x]["priority"]
        )
    ...
    def _synthesize_sequential(self, outputs: List[Dict], state: AgentState) -> str:
        ...
        for i, output in enumerate(outputs[:-1]):
            ...
            insights.append(f"**{self._get_agent_display_name(agent_name)}的见解:**\n{content}")
    ...
    def _synthesize_hierarchical(self, outputs: List[Dict], state: AgentState) -> str:
        ...
        sorted_outputs = sorted(
            outputs,
            key=lambda x: self.agent_capabilities.get(x.get("agent", ""), {}).get("priority", 99)
        )
    ...
    def _generate_integration_summary(self, outputs: List[Dict], state: AgentState) -> str:
        """生成整合性总结"""
        # 这里可以使用LLM来生成更智能的总结
        # 暂时使用模板化方法
...
每位专家都从自己的专业领域出发，为您提供了详细的分析和建议。请查看下方各专家的具体建议，并根据您的实际情况进行选择和整合。
    ...
    def _calculate_quality_score(self, state: AgentState) -> Optional[float]:
        ...
        try:
            ...
            completion_score = completed_agents / selected_agents if selected_agents > 0 else 0
            score_factors.append(completion_score * 3)  # 权重3
            
            # 2. 响应质量评分（基于输出长度和结构）
            agent_outputs = [msg for msg in state.messages if msg.get("role") == "assistant"]
            if agent_outputs:
                avg_length = sum(len(msg["content"]) for msg in agent_outputs) / len(agent_outputs)
