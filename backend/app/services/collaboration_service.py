...
class CollaborationService:
    ...
    async def remove_collaborator(
        self, db: AsyncSession, course_id: UUID, user_id: UUID, removed_by: UUID = None
    ) -> Dict[str, Any]:
        """移除协作者"""
        ...
        return {"course_id": course_id, "user_id": user_id, "message": "协作者移除成功"}
        ...
        result = await db.execute(
            ...
            .select_from(
                course_collaborators.join(
                    User, course_collaborators.c.user_id == User.id
                )
            )
    ...
    async def check_permission(
        self, db: AsyncSession, course_id: UUID, user_id: UUID, permission: Permission
    ) -> bool:
        """检查用户权限"""
    ...
    async def share_course(
        ...
        return {
            ...
            "share_url": self._generate_share_url(
                course_id, share_config.get("share_token")
            ),
            "message": "课程分享成功",
        }

    async def get_shared_courses(
        self, db: AsyncSession, user_id: UUID, share_scope: Optional[ShareScope] = None
    ) -> List[Dict[str, Any]]:
        """获取用户有权访问的共享课程"""
        ...
        for course in courses:
            ...
            share_config = (
                course.metadata.get("share_config", {}) if course.metadata else {}
            )
    ...
    async def get_course_activity_log(
        self, db: AsyncSession, course_id: UUID, limit: int = 50
    ) -> List[Dict[str, Any]]:
        """获取课程活动日志"""
    ...
    def _generate_share_url(
        self, course_id: UUID, share_token: Optional[str] = None
    ) -> str:
        """生成分享链接"""
